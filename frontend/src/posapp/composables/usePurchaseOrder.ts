...
export function usePurchaseOrder(options: {
    ...
    const { posProfile, receiveNow, formatFloat } = options;
    ...
    const totalAmount = computed(() => {
        return purchaseItems.value.reduce((sum, item) => sum + item.qty * item.rate, 0);
    });
    ...
    const onAddItem = async (item: any) => {
        ...
        const existingItem = purchaseItems.value.find((p) => p.item_code === item.item_code);
    ...
    const updateItemUom = async (item: PurchaseItem, value: string) => {
        ...
        item.uom = value;
        const matched = (item.item_uoms || []).find((uom: any) => uom.uom === value);
    ...
    const removeItem = (item: PurchaseItem) => {
        purchaseItems.value = purchaseItems.value.filter((row) => row.line_id !== item.line_id);
    };
    ...
    const resetForm = () => {
        ...
        transactionDate.value = formatUtils.toArabicNumerals(frappe.datetime.nowdate());
        scheduleDate.value = formatUtils.toArabicNumerals(frappe.datetime.nowdate());
