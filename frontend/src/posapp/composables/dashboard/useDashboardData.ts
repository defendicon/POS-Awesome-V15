...
import {
	fetchDashboardData as fetchDashboardDataService,
	type DashboardRequest,
	type DashboardResponse,
} from "@/posapp/services/dashboardService";

export type DashboardScope = "all" | "current" | "specific";

export interface DashboardRequestSources {
	profileName: ComputedRef<string>;
	dashboardScope: Ref<DashboardScope>;
	selectedProfileFilter: Ref<string>;
	selectedReportMonth: Ref<string>;
	configuredLowStockThreshold: ComputedRef<number | undefined>;
	itemSalesLimit: Ref<number>;
	categoryReportLimit: Ref<number>;
	inventoryStatusLimit: Ref<number>;
...
export interface DashboardDataOptions {
	fetchDashboardData?: (args?: DashboardRequest) => Promise<DashboardResponse>;
	isDebugEnabled?: () => boolean;
	logDebug?: (event: DashboardLogEvent) => void;
...
export function buildDashboardRequest(
	sources: DashboardRequestSources,
): DashboardRequest {
	return {
		pos_profile: sources.profileName.value || undefined,
		scope: sources.dashboardScope.value,
		profile_filter:
			sources.dashboardScope.value === "specific"
				? sources.selectedProfileFilter.value || undefined
				: undefined,
		report_month: sources.selectedReportMonth.value || undefined,
		low_stock_threshold: sources.configuredLowStockThreshold.value,
		item_sales_limit: sources.itemSalesLimit.value,
		category_report_limit: sources.categoryReportLimit.value,
		inventory_status_limit: sources.inventoryStatusLimit.value,
		stock_movement_limit: sources.stockMovementLimit.value,
		reorder_suggestion_limit: sources.reorderSuggestionLimit.value,
		payment_report_limit: sources.paymentReportLimit.value,
		discount_report_limit: sources.discountReportLimit.value,
		customer_report_limit: sources.customerReportLimit.value,
		staff_report_limit: sources.staffReportLimit.value,
		profitability_report_limit: sources.profitabilityReportLimit.value,
		branch_report_limit: sources.branchReportLimit.value,
		tax_report_limit: sources.taxReportLimit.value,
		fast_moving_page: sources.fastMovingPage.value,
		fast_moving_page_size: sources.fastMovingPageSize.value,
		fast_moving_search: sources.fastMovingSearch.value || undefined,
	};
}

function defaultDashboardDebugLogger(event: DashboardLogEvent) {
	const prefix = "[AwesomeDashboard]";
	if (event.type === "request") {
		console.groupCollapsed(`${prefix} fetch:start`);
		console.info(event.payload);
		console.groupEnd();
		return;
	}
	if (event.type === "response") {
		console.groupCollapsed(`${prefix} fetch:success`);
		console.info(event.payload);
		console.groupEnd();
		return;
	}
	console.groupCollapsed(`${prefix} fetch:error`);
...
export function useDashboardData(
	...
	const fetchDashboardData = options.fetchDashboardData || fetchDashboardDataService;
	const isDebugEnabled = options.isDebugEnabled || (() => false);
	const logDebug = options.logDebug || defaultDashboardDebugLogger;
	const translate = options.translate || ((value: string) => value);
	const now = options.now || (() => new Date());

	const emitDebug = (event: DashboardLogEvent) => {
		if (isDebugEnabled()) {
			logDebug(event);
		}
	};
	...
	const loadDashboard = async () => {
		loading.value = true;
		errorMessage.value = "";
		const request = buildDashboardRequest(sources);
		emitDebug({ type: "request", payload: request });

		try {
			const response = await fetchDashboardData(request);
			emitDebug({ type: "response", payload: response });
			dashboardData.value = mergeDashboardPayload(response);
			if (response.date_context?.report_month) {
				sources.selectedReportMonth.value = String(
					response.date_context.report_month,
				);
			}
			...
			if (!scopeInitialized.value) {
				sources.dashboardScope.value = (response.default_scope ||
					sources.dashboardScope.value) as DashboardScope;
				scopeInitialized.value = true;
			}
			if (
				!allowAllProfiles.value &&
				sources.dashboardScope.value !== "current"
			) {
				sources.dashboardScope.value = "current";
			}
			if (
				sources.dashboardScope.value === "specific" &&
				!sources.selectedProfileFilter.value
			) {
				sources.selectedProfileFilter.value =
					response.available_profiles?.find(
						(profile) => profile.dashboard_enabled !== false,
					)?.name ||
					response.selected_profiles?.[0] ||
					"";
			}
			lastUpdatedAt.value = now();
		} catch (error: any) {
			emitDebug({ type: "error", payload: error });
			errorMessage.value =
				error?.message || translate("Failed to load dashboard data.");
		} finally {
			loading.value = false;
		}
	};
