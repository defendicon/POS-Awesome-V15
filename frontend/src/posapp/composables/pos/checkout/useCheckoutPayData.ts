...
export function usePosPayData({
	...
	async function get_outstanding_invoices(
		...
		try {
			...
			const r = await frappe.call(
				"posawesome.posawesome.api.checkout_entry.get_outstanding_invoices",
				{
	...
	async function get_unallocated_payments() {
		...
		try {
			const r = await frappe.call(
				"posawesome.posawesome.api.checkout_entry.get_unallocated_payments",
				{
					customer: customerName.value,
					party: customerName.value,
					party_type: resolvedPartyType,
	...
	async function autoReconcile(
		...
		try {
			const response = await frappe.call({
				method: "posawesome.posawesome.api.checkout_entry.auto_reconcile_customer_invoices",
				args: {
					customer: customerName.value,
					party_type: resolvedPartyType,
					company: company.value,
