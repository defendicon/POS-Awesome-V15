...
import { openItemSelectionDialog } from "../../../utils/catalogSelectionDialog";
// @ts-ignore
...
export function useScanProcessor(context: ScanProcessorContext) {
			...
			if (barcodeMatch && barcodeMatch.posa_uom) {
				...
				try {
					const res = await frappe.call({
						method: "posawesome.posawesome.api.catalog.get_price_for_uom",
						args: {
	...
	const processScannedItem = async (scannedCode: string) => {
		...
		try {
			const res = await frappe.call({
				method: "posawesome.posawesome.api.catalog.parse_scale_barcode",
			...
			if (searchSerialNo || searchBatchNo) {
				try {
					const resolveRes = await frappe.call({
						method: "posawesome.posawesome.api.catalog.search_serial_or_batch_or_barcode_number",
						args: {
		...
		try {
			...
			if (qtyFromBarcode !== null) {
				...
				const res = await frappe.call({
					method: "posawesome.posawesome.api.catalog.get_item_detail",
					args: {
