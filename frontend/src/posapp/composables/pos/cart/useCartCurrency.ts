...
export function useInvoiceCurrency() {
	...
	const fetch_available_currencies = async () => {
		...
		try {
			const r = await frappe.call({
				method: "posawesome.posawesome.api.cart_management.get_available_currencies",
			});
	...
	const update_currency_and_rate = async () => {
		...
		try {
			...
			if (selected_currency.value === plCurrency) {
				...
				const r = await frappe.call({
					method: "posawesome.posawesome.api.cart_management.fetch_exchange_rate_pair",
					args: {
	...
	const fetch_price_lists = async () => {
		...
		try {
			const r = await frappe.call({
				method: "posawesome.posawesome.api.cart_management.get_price_list_currency",
