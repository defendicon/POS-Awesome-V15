...
export function useScannerInput(options: ScannerInputOptions = {}) {
    ...
    const setScanHandler = (fn: (code: string) => Promise<void> | void) => { onScanHandler.value = fn; };
    const setInputHandlers = (handlers: { get?: () => string; set?: (val: string) => void; clear?: () => void; focus?: () => void }) => {
    ...
    // --- Error Handling ---
    const showScanError = (options: { message?: string; code?: string; details?: string } = {}) => {
        scanErrorMessage.value = options.message || __("Unable to add scanned item.");
        ...
        scanDebounceId.value = setTimeout(() => {
            ...
            scheduleFrame(() => {
                ...
                if (maybePromise && typeof maybePromise.catch === "function") {
                    maybePromise.catch((e: any) => handleScanPipelineError(e, code));
                }
            });
        }, 12);
    };
    ...
    const handleSearchKeydown = (event: KeyboardEvent) => {
        ...
        const currentVal = getSearchInputHandler.value ? (getSearchInputHandler.value as any)() : "";
        ...
        keyboardScanTimer.value = setTimeout(() => {
            evaluateKeyboardScan(getSearchInputHandler.value ? (getSearchInputHandler.value as any)() : "");
    ...
    const handleSearchPaste = (event: ClipboardEvent) => {
        ...
        if (isScanCandidate(sanitized, keyboardScanMinLength)) {
            event.preventDefault();
            if (setSearchInputHandler.value) (setSearchInputHandler.value as any)(sanitized);
    ...
    onUnmounted(() => {
        ...
        if ((document as any)._scannerAttached && typeof onScan !== "undefined") {
            try {
    ...
    return {
        ...
        scaleBarcodeMatches: (val: string) => scaleBarcodeMatches(scaleBarcodeSettings.value, val),
        setScanHandler,
        setInputHandlers,
    };
}
