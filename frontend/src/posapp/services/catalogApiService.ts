...
const itemService = {
  getItemGroups(): Promise<ItemGroup[]> {
    return api.call("posawesome.posawesome.api.catalog.get_items_groups");
  },

  getItems(args: GetItemsArgs, signal?: AbortSignal): Promise<Item[]> {
    return api.call("posawesome.posawesome.api.catalog.get_items", args, { signal });
  },

  getItemsFromBarcode(args: { selling_price_list: string; currency: string; barcode: string }): Promise<Item | null> {
    return api.call("posawesome.posawesome.api.catalog.get_items_from_barcode", args);
  },

  getItemBrand(itemCode: string): Promise<string> {
    return api.call("posawesome.posawesome.api.catalog.get_item_brand", { item_code: itemCode });
  },
