...
.scan-line {
  ...
  animation: scan-enhanced 2s linear infinite; box-shadow: 0 0 10px rgba(76, 175, 80, 0.5);
}
...
.scan-corners { position: absolute; top: 20%; left: 20%; right: 20%; bottom: 20%; }
.corner {
  position: absolute; width: 20px; height: 20px; border: 3px solid #4caf50;
  animation: pulse-corners 2s ease-in-out infinite; box-shadow: 0 0 5px rgba(76, 175, 80, 0.3);
}
...
.status-messages { background: rgba(255, 255, 255, 0.95); }
.camera-scanner-dialog { align-self: flex-start; justify-self: flex-end; margin: 16px; }
.scanner-container:hover .scanning-overlay { opacity: 0.9; }
.scanner-container .scanning-overlay { transition: opacity 0.3s ease; }
</style>
...
export default {
  ...
  methods: {
    ...
    handleScannedCode(rawValue, formatLabel = "", options = {}) {
      ...
      if (closeDialog) {
        const effectiveDelay = Math.max(0, typeof closeDelay === "number" ? closeDelay : Math.min(resetDelay, 250));
    ...
    onError(error) {
      ...
      if (error.name === "NotAllowedError") {
        ...
        this.errorMessage = this.__("Camera constraints not supported by your device. Trying fallback settings...");
    ...
    async tryFallbackCamera() {
      ...
      try {
        ...
        if (typeof frappe !== "undefined" && frappe.show_alert) {
          frappe.show_alert(
            { message: this.__("Using basic camera settings due to device limitations"), indicator: "orange" },
            3,
          );
        }
      } catch (fallbackError) {
