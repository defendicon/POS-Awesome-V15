...
def get_dashboard_data(
    ...
    limits = normalize_dashboard_limits(
        fast_moving_limit=fast_moving_limit,
        ...
        coerce_text=lambda value: cstr(value).strip(),
    )
    ...
    profile_selection = resolve_selected_profiles(
        ...
        current_profile_doc=current_profile_doc,
        is_dashboard_enabled=_is_dashboard_enabled,
    )
    available_profiles = profile_selection.available_profiles
    selected_profiles = profile_selection.selected_profiles
    selected_profile_names = profile_selection.selected_profile_names
    ...
    payload = build_dashboard_payload(
        DashboardPayloadContext(
            ...
            disabled_reason=profile_selection.disabled_reason,
            ...
            report_to_date=str(report_to_date),
            month_start=str(month_start),
            selected_report_month=selected_report_month,
            threshold=threshold,
            fast_moving_days=fast_moving_days,
            fast_moving_page=limits.fast_moving_page,
            fast_moving_page_size=limits.fast_moving_page_size,
            fast_moving_search=limits.fast_moving_search,
            sales_trend_week_from=str(report_to_date - timedelta(days=55)),
            sales_trend_month_from=str(getdate(add_months(report_to_date, -5)).replace(day=1)),
        )
    )
    ...
    payload["payment_method_report"] = _collect_payment_method_report(
        ...
        date_from=str(month_start),
        date_to=str(report_to_date),
        limit=limits.payment_report_limit,
    )
    payload["discount_void_return_report"] = _collect_discount_void_return_report(
        ...
        date_from=str(month_start),
        date_to=str(report_to_date),
        limit=limits.discount_report_limit,
    )
    payload["customer_report"] = _collect_customer_report(
        ...
        date_from=str(month_start),
        date_to=str(report_to_date),
        limit=limits.customer_report_limit,
    )
    payload["staff_performance_report"] = _collect_staff_cashier_performance_report(
        ...
        date_from=str(month_start),
        date_to=str(report_to_date),
        limit=limits.staff_report_limit,
    )
    payload["profitability_report"] = _collect_profitability_report(
        ...
        date_from=str(month_start),
        date_to=str(report_to_date),
        limit=limits.profitability_report_limit,
    )
    payload["branch_location_report"] = _collect_branch_location_report(
        ...
        date_to=str(report_to_date),
        threshold=threshold,
        limit=limits.branch_report_limit,
    )
    ...
    payload["category_brand_variant_report"] = _collect_category_brand_variant_report(
        ...
        date_from=str(month_start),
        date_to=str(report_to_date),
        limit=limits.category_report_limit,
    )
    payload["inventory_status_report"] = _collect_inventory_status_report(
        ...
        date_from=str(month_start),
        date_to=str(report_to_date),
        limit=limits.inventory_status_limit,
    )
    payload["stock_movement_report"] = _collect_stock_movement_report(
        ...
        date_from=str(month_start),
        date_to=str(report_to_date),
        limit=limits.stock_movement_limit,
    )
    payload["reorder_purchase_suggestions"] = _collect_reorder_purchase_suggestions(
        ...
        date_from=str(month_start),
        date_to=str(report_to_date),
        limit=limits.reorder_suggestion_limit,
    )
    ...
    fast_moving_items, fast_moving_total_count = _collect_fast_moving_items(
        ...
        date_from=str(month_start),
        date_to=str(report_to_date),
        limit=limits.fast_moving_page_size,
        offset=limits.fast_moving_offset,
        search_text=limits.fast_moving_search,
    )
    payload["inventory_insights"]["fast_moving_items"] = fast_moving_items
    update_fast_moving_pagination(
        payload,
        page=limits.fast_moving_page,
        page_size=limits.fast_moving_page_size,
        total_count=fast_moving_total_count,
        search=limits.fast_moving_search,
    )
    payload["inventory_insights"]["low_stock_items"] = _collect_low_stock_items(
        warehouses=warehouses,
        threshold=threshold,
        limit=limits.low_stock_limit,
    )
    payload["supplier_overview"] = _collect_supplier_overview_report(
        company=company,
        date_from=str(month_start),
        date_to=str(report_to_date),
        limit=limits.supplier_limit,
    )
